...
void SortLambda(T* items, nint length, F orderer)
{
	if (length == 0)
	{
		return;
...
// template<typename T>
// void Sort(T* items, nint length, const Func<nint(T, T)>& orderer)
// {
// 	SortLambda<T, Func<nint(T, T)>>(items, length, orderer);
// }
...
class LazyList : public Object, public IEnumerable<T>
{
	...
	public:
		...
		LazyList<T> Take(nint count)const
		{
		...
		/// <returns>The created lazy list.</returns>
		/// <param name="count">The size of the prefix.</param>
		LazyList<T> Skip(nint count)const
		{
		...
		LazyList<T> Repeat(nint count)const
		{
		...
		template<typename U>
		LazyList<NPair<T, U>> Pairwise(const IEnumerable<U>& remains)const
		{
		...
		// 		template<typename F>
		// 		LazyList<NPair<FUNCTION_RESULT_TYPE(F), LazyList<T>>> GroupBy(F f)const
		// 		{
