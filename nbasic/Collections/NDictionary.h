...
template <
  ...
  >
class NDictionary : public Object, public virtual IEnumerable<NPair<KT, VT>>
{
	public:
	...
	protected:
		...
		{
			private:
				const NDictionary<KT, VT, KK, VK>*	container;
				nint								index;
				NPair<KT, VT>						current;
			...
			public:
				Enumerator(const NDictionary<KT, VT, KK, VK>* _container, nint _index = -1)
				{
...
template <
  ...
  typename VK = typename KeyType<VT>::Type
  >
class NGroup : public Object, public virtual IEnumerable<NPair<KT, VT>>
{
		typedef NSortedList<KT, KK>		KeyContainer;
		typedef NList<VT, VK>			ValueContainer;
	protected:
		...
		{
			private:
				const NGroup<KT, VT, KK, VK>*		container;
				nint								keyIndex;
				nint								valueIndex;
				NPair<KT, VT>						current;
			...
			public:
				Enumerator(const NGroup<KT, VT, KK, VK>* _container, nint _keyIndex = -1, nint _valueIndex = -1)
				{
					container = _container;
					keyIndex = _keyIndex;
...
template <
  ...
  >
void GroupInnerJoin(
  const NGroup<TKey, TValueFirst>& first,
  const NGroup<TKey, TValueSecond>& second,
...
struct RandomAccessable<NDictionary<KT, VT, KK, VK>>
{
...
template<typename KT, typename VT, typename KK, typename VK>
struct RandomAccess<NDictionary<KT, VT, KK, VK>>
{
	static nint GetCount(const NDictionary<KT, VT, KK, VK>& t)
	{
		return t.Count();
	}
	
	static NPair<KT, VT> GetValue(const NDictionary<KT, VT, KK, VK>& t, nint index)
	{
		return NPair<KT, VT>(t.Keys().Get(index), t.Values().Get(index));
	}
	
	static void AppendValue(NDictionary<KT, VT, KK, VK>& t, const NPair<KT, VT>& value)
	{
