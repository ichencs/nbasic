...
bool WaitableObject::WaitForTime(nint ms)
{
	if (IsCreated())
	{
...
bool WaitableObject::WaitAll(WaitableObject** objects, nint count)
{
	NArray<HANDLE> handles(count);
...
bool WaitableObject::WaitAllForTime(WaitableObject** objects, nint count, nint ms)
{
	NArray<HANDLE> handles(count);
...
nint WaitableObject::WaitAny(WaitableObject** objects, nint count, bool* abandoned)
{
	NArray<HANDLE> handles(count);
...
nint WaitableObject::WaitAnyForTime(WaitableObject** objects, nint count, nint ms, bool* abandoned)
{
	NArray<HANDLE> handles(count);
...
void Thread::Sleep(nint ms)
{
	::Sleep((DWORD)ms);
}


nint Thread::GetCPUCount()
{
	SYSTEM_INFO info;
	GetSystemInfo(&info);
...
nint Thread::GetCurrentThreadId()
{
...
void Thread::SetCPU(nint index)
{
	SetThreadAffinityMask(internalData->handle, ((nint)1 << index));
}
...
bool Semaphore::Create(nint initialCount, nint maxCount, const WString& name)
{
	if (IsCreated())
	{
		return false;
...
nint Semaphore::Release(nint count)
{
	if (IsCreated())
	{
		LONG previous = -1;
...
bool ConditionVariable::SleepWithForTime(CriticalSection& cs, nint ms)
{
...
bool ConditionVariable::SleepWithReaderForTime(ReaderWriterLock& lock, nint ms)
{
...
bool ConditionVariable::SleepWithWriterForTime(ReaderWriterLock& lock, nint ms)
{
