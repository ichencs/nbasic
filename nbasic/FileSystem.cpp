...
bool NFile::ReadAllTextWithEncodingTesting(WString& text, BomEncoder::Encoding& encoding, bool& containsBom)
{
	...
	{
		NFileStream fileStream(filePath.GetFullPath(), NFileStream::ReadOnly);
...
{
	NFileStream fileStream(filePath.GetFullPath(), NFileStream::WriteOnly);
