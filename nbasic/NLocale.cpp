...
DWORD TranslateNormalization(NLocale::Normalization normalization)
{
	...
	if (normalization & NLocale::IgnoreCase)
	{
		result |= NORM_IGNORECASE;
	}
	
	if (normalization & NLocale::IgnoreCaseLinguistic)
	{
		result |= NORM_IGNORECASE | NORM_LINGUISTIC_CASING;
	}
	
	if (normalization & NLocale::IgnoreKanaType)
	{
		result |= NORM_IGNOREKANATYPE;
	}
	
	if (normalization & NLocale::IgnoreNonSpace)
	{
		result |= NORM_IGNORENONSPACE;
	}
	
	if (normalization & NLocale::IgnoreSymbol)
	{
		result |= NORM_IGNORESYMBOLS;
	}
	
	if (normalization & NLocale::IgnoreWidth)
	{
		result |= NORM_IGNOREWIDTH;
	}
	
	// 		if(normalization&Locale::DigitsAsNumbers) result|=SORT_DIGITSASNUMBERS;
	if (normalization & NLocale::StringSoft)
	{
