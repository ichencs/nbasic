...
Automaton::Ref EpsilonNfaToNfa(Automaton::Ref source, bool(*epsilonChecker)(Transition*), NDictionary<State*, State*>& nfaStateMap)
{
	Automaton::Ref target = new Automaton;
	NDictionary<State*, State*> stateMap;	//source->target
	NList<State*> epsilonStates;				//每次迭代当前状态的epsilon闭包
	NList<Transition*> transitions;			//每次迭代当前状态的epsilon闭包的转换集合
...
Automaton::Ref NfaToDfa(Automaton::Ref source, NGroup<State*, State*>& dfaStateMap)
{
	Automaton::Ref target = new Automaton;
	NGroup<Transition*, Transition*> nfaTransitions;
	NList<Transition*> transitionClasses;//保证转换先后顺序不被nfaTransitions.Keys破坏
